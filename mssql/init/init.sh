...
DB_EXISTS=$(/opt/mssql-tools/bin/sqlcmd -S localhost -U sa -P "$SA_PASSWORD" -d master \
  -Q "SET NOCOUNT ON; SELECT CASE WHEN DB_ID('ClothingStore') IS NOT NULL THEN 1 ELSE 0 END" -h -1 -W | tr -d '\r\n')

if [ "$DB_EXISTS" -eq 0 ]; then
    echo "Database chưa tồn tại, đang chạy script khởi tạo..."
    /opt/mssql-tools/bin/sqlcmd -S localhost -U sa -P "$SA_PASSWORD"  -d master -i /init/restore.sql
else
    echo "Database ClothingStore đã tồn tại, bỏ qua khởi tạo"
fi
